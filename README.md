...
- **两阶段训练策略**: 严格遵循论文中的方法，先仅使用数据损失进行预训练，然后加入物理损失进行联合优化，以保证训练的稳定性和收敛性。
- **敏感性分析**: 集成 `SALib` 库，将训练好的PINN作为高效的代理模型进行Sobol全局敏感性分析，极大提升了分析效率。
- **现代化工具链**: 使用 `uv` 进行环境和依赖管理，提供闪电般的安装速度和流畅的开发体验。
...
脚本将依次执行以下步骤：
1.  **生成模拟数据**: 用于演示目的，实际使用时可替换为您自己的数据集。
...
- **`calculate_physics_residuals()` 函数**:
  - **物理核心**，此函数用于定义热工水力学的控制方程（示例中为简化的能量守恒方程）。您需要根据您的具体问题，在此处编写精确的物理方程残差计算逻辑。
...
- **`predict_with_uncertainty()` 函数**:
  - 实现不确定性量化的核心函数。通过将模型置于 `train()` 模式（以激活Dropout）并进行多次前向传播来收集预测样本。
